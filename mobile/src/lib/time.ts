...
export function todayDateString(): string {
  const now = new Date();
  return `${DAY_NAMES[now.getDay()]}, ${MONTH_NAMES_SHORT[now.getMonth()]} ${now.getDate()}, ${now.getFullYear()}`;
}
...
export function weekStartDateFor(date: Date, weekStartDay: number): string {
  const d = new Date(date);
  const dayIdx = d.getDay();
  const offset = weekStartDay === 0
    ? -dayIdx
    : (dayIdx === 0 ? -6 : 1 - dayIdx);
