...
export const useScheduleStore = create<ScheduleStore>((set, get) => ({
  ...
  async load() {
    try {
      set({ error: null });
      const payload = await ScheduleService.getSchedule();
      set({
  ...
  async createTemplate() {
    ...
    try {
      set({ error: null });
      const templateId = await ScheduleService.createTemplate(name);
      await ScheduleService.activateTemplate(templateId);
  ...
  async saveDayTargets() {
    ...
    try {
      set({ error: null });
      await ScheduleService.saveDayTargets(activeTemplateId, get().dayTargets);
    } catch (error) {
